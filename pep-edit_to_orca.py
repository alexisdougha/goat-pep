...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prepare ORCA input files from PEP-EDIT outputs. You only need to provide the PDB and SMILES files generated by PEP-EDIT."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "pep_edit_smi_path",
        type=str,
        help="Path to the PEP-EDIT generated SMILES file.",
    )
    parser.add_argument(
        "output_dir",
        type=str,
        help="Directory to save the ORCA input file and XYZ file.",
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    parser.add_argument(
        "--alpb_water",
        action="store_true",
        help="Whether to use the ALPB model for water.",
    )
    parser.add_argument(
        "--nprocs", type=int, default=8, help="The number of processors to use."
    )
    parser.add_argument(
        "--maxcore",
        type=int,
        default=23000,
        help="The maximum amount of memory to use (in MB).",
    )

    args = parser.parse_args()

    prepare_orca_inp_file(
        args.pep_edit_pdb_path,
        args.pep_edit_smi_path,
        args.output_dir,
        args.method,
        args.alpb_water,
        args.nprocs,
